...
reportOmvClass <- if (requireNamespace("jmvcore", quietly = TRUE)) R6::R6Class(
    ...
    private = list(
        .init = function() {
            ...
            self$results$list$setTitle("Analyses in the file")
            self$results$tables$setTitle("Recomputed tables")
        ...
        .run = function() {
            ...
            if (!isTRUE(self$options$run)) {
                self$results$info$setContent("<p>Run your analyses in jamovi and save the file (.omv). Then set the options and tick <b>Generate report</b>. If the file field is empty, the most recently saved .omv in Documents/Desktop/Downloads is used automatically.</p>")
            ...
            self_ <- self
            rep <- report_from_omv(
                file = self$options$file, lang = lang, useLLM = self$options$useLLM, backend = self$options$backend,
            ...
            if (!is.null(rep$interp)) self$results$interp$setContent(paste0(rep$interp, "<p style='color:#777;font-size:90%'>", sprintf(L$interp_note, html_escape(rep$provider)), "</p>"))
